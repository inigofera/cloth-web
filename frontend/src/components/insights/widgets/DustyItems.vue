<template>
  <div v-if="rows.length" class="dusty-list">
    <div v-for="r in rows" :key="r.id" class="dusty-row">
...
import { dustyItems, rangeData, widgetRange, type DustyOptions } from '../../../lib/insights/metrics';
...
const includeNeverWorn = computed(() => props.config.includeNeverWorn !== false);

const sortBy = computed<DustyOptions['sortBy']>(() => {
  const v = props.config.sortBy;
  return v === 'price' || v === 'recent' ? v : 'days';
});
...
const rows = computed(() =>
  dustyItems(data.value, threshold.value, {
    includeNeverWorn: includeNeverWorn.value,
    sortBy: sortBy.value,
    fullData: props.data,
  }).slice(0, topN.value),
);
